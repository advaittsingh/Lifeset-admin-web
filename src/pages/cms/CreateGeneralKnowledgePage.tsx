...
export default function CreateGeneralKnowledgePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    date: '', // Event date
    eventDates: [] as Array<{ date: string; time: string; title: string }>, // Array of notification dates with times and titles
    eventYearRange: '',
    location: {
      lat: '',
      long: '',
  ...
  const { data: subCategoriesData } = useQuery({
    queryKey: ['gk-subcategories', formData.categoryId],
    queryFn: () =>
  ...
  // This should only run in create mode, not edit mode, and only once
  const [urlParamsInitialized, setUrlParamsInitialized] = useState(false);
  useEffect(() => {
    if (!isEditMode && !urlParamsInitialized) {
  ...
  }, [searchParams, isEditMode, urlParamsInitialized]);
  ...
  const { data: chaptersData } = useQuery<Chapter[]>({
    queryKey: ['gk-chapters', formData.subCategoryId],
    queryFn: () =>
  ...
  const { data: mcqSubCategoriesData } = useQuery({
    queryKey: ['gk-mcq-subcategories', mcqFormData.categoryId],
    queryFn: () =>
      mcqFormData.categoryId ? cmsApi.getSubCategories(mcqFormData.categoryId, 'general-knowledge') : [],
  ...
  const { data: mcqChaptersData } = useQuery<Chapter[]>({
    queryKey: ['gk-mcq-chapters', mcqFormData.subCategoryId],
    queryFn: () =>
      mcqFormData.subCategoryId ? cmsApi.getChaptersBySubCategoryNew(mcqFormData.subCategoryId, 'general-knowledge') : [],
  ...
  const { data: existingItem, isLoading: isLoadingItem, error: existingItemError } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      try {
        console.log('Fetching general knowledge item with ID:', id);
        const item = await cmsApi.getGeneralKnowledgeById(id);
  ...
  useEffect(() => {
    if (existingItem && isEditMode && !isLoadingItem && typeof existingItem === 'object' && dataLoadedKey === 'initial') {
      ...
      // Support both old format (string[]) and new format (Array<{date, time, title}>)
      let eventDates: Array<{ date: string; time: string; title: string }> = [];
      if (item.eventDates) {
        if (Array.isArray(item.eventDates)) {
          ...
          if (item.eventDates.length > 0 && typeof item.eventDates[0] === 'object' && item.eventDates[0] !== null) {
            // New format: ensure time field exists
            eventDates = (item.eventDates as any[]).map(item => ({
              date: item.date || '',
              time: item.time || '',
              title: item.title || ''
            }));
            ...
            eventDates = (item.eventDates as string[]).map(date => ({ date, time: '', title: '' }));
          }
        }
        ...
        if (Array.isArray(metadata.eventDates)) {
          ...
          if (metadata.eventDates.length > 0 && typeof metadata.eventDates[0] === 'object' && metadata.eventDates[0] !== null) {
            // New format: ensure time field exists
            eventDates = (metadata.eventDates as any[]).map(item => ({
              date: item.date || '',
              time: item.time || '',
              title: item.title || ''
            }));
            ...
            // Old format: convert strings to objects
            eventDates = (metadata.eventDates as string[]).map(date => ({ date, time: '', title: '' }));
          }
          ...
          try {
            ...
            if (Array.isArray(parsed)) {
              if (parsed.length > 0 && typeof parsed[0] === 'object' && parsed[0] !== null) {
                // New format: ensure time field exists
                eventDates = (parsed as any[]).map(item => ({
                  date: item.date || '',
                  time: item.time || '',
                  title: item.title || ''
                }));
              } else {
                eventDates = (parsed as string[]).map(date => ({ date, time: '', title: '' }));
              }
            }
          } catch {
            eventDates = [];
      ...
      const formDataToSet = {
        ...
        postType: item.postType || metadata.postType || 'General Knowledge',
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof formData) => {
      ...
      const payload: any = {
        ...
        articleType: 'GENERAL_KNOWLEDGE', // Required for GK articles
      ...
      console.log('Creating general knowledge with payload:', JSON.stringify(payload, null, 2));
      
      return cmsApi.createGeneralKnowledge(payload);
    },
    onSuccess: async (response) => {
      // Invalidate and refetch to update counts
      ...
      if (formData.sendNotification && createdArticle) {
        try {
          const imageUrl = formData.imagePreview || formData.imageUrl || (createdArticle.images && createdArticle.images[0]);
          ...
          const plainTextDescription = stripHtml(formData.description || 'Check out our latest general knowledge content!');
          ...
          if (formData.notificationTime && formData.notificationTime.trim()) {
            ...
            await notificationJobsApi.create({
              messageType: 'ARTICLE',
              title: formData.title || 'New General Knowledge Article',
              content: notificationBody || 'Check out our latest general knowledge content!',
              image: imageUrl || undefined,
              redirectionLink: `https://lifeset.app/general-knowledge/${articleId}`,
            ...
            await notificationsApi.send({
              ...
              notification: {
                title: formData.title || 'New General Knowledge Article',
                body: notificationBody || 'Check out our latest general knowledge content!',
              },
              redirectUrl: `https://lifeset.app/general-knowledge/${articleId}`,
          ...
          if (errorMessage.includes('No tokens found') || errorMessage.includes('no tokens')) {
            showToast('Article created. Note: No users have registered push tokens yet.', 'warning');
          } else {
            showToast(`Article created but notification failed: ${errorMessage}`, 'error');
          }
        }
      }
    ...
    onError: (error: any) => {
      ...
      console.error('Create general knowledge error:', {
        error,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: typeof formData) => {
      ...
      // Log payload for debugging
      console.log('Updating general knowledge with payload:', JSON.stringify(payload, null, 2));
      
      return cmsApi.updateGeneralKnowledge(id!, payload);
    },
    onSuccess: async () => {
      // Invalidate and refetch to update counts
      await queryClient.invalidateQueries({ queryKey: ['general-knowledge'] });
      await queryClient.invalidateQueries({ queryKey: ['general-knowledge-item', id] });
      await queryClient.invalidateQueries({ queryKey: ['wall-categories'] });
      await queryClient.invalidateQueries({ queryKey: ['chapter-post-counts'] });
      await queryClient.refetchQueries({ queryKey: ['wall-categories', 'parents'] });
      ...
      if (formData.sendNotification && id) {
        try {
          ...
          const plainTextDescription = stripHtml(formData.description || 'Check out our updated general knowledge content!');
          const notificationBody = plainTextDescription.length > 200 
            ? plainTextDescription.substring(0, 200) + '...' 
            : plainTextDescription;
          ...
          if (formData.notificationTime && formData.notificationTime.trim()) {
            ...
            await notificationJobsApi.create({
              messageType: 'ARTICLE',
              title: formData.title || 'Updated General Knowledge Article',
              content: notificationBody || 'Check out our updated general knowledge content!',
              image: imageUrl || undefined,
              redirectionLink: `https://lifeset.app/general-knowledge/${id}`,
              scheduledAt: scheduledDate.toISOString(),
              language: 'ALL',
              frequency: 'ONCE',
            });
            ...
            await notificationsApi.send({
              ...
              notification: {
                title: formData.title || 'Updated General Knowledge Article',
                body: notificationBody || 'Check out our updated general knowledge content!',
              },
              redirectUrl: `https://lifeset.app/general-knowledge/${id}`,
              imageUrl: imageUrl || undefined,
              data: {
                articleId: id,
                type: 'general-knowledge',
          ...
          // Check if it's a "no tokens" error - this is expected if users haven't registered tokens yet
          if (errorMessage.includes('No tokens found') || errorMessage.includes('no tokens')) {
            showToast('Article updated. Note: No users have registered push tokens yet.', 'warning');
          } else {
            showToast(`Article updated but notification failed: ${errorMessage}`, 'error');
          }
        }
      }
    ...
    onError: (error: any) => {
      ...
      console.error('Update general knowledge error:', {
        error,
        response: error?.response,
        responseData: errorResponse,
        requestData: requestPayload,
      ...
      // Extract error message (same logic as createMutation)
      ...
      if (error?.response?.data) {
        ...
        if (responseData.error) {
          ...
          if (Array.isArray(responseData.details)) {
            errorMessage = responseData.details
              .map((detail: any) => typeof detail === 'string' ? detail : JSON.stringify(detail))
              .join(', ');
          } else if (typeof responseData.details === 'string') {
            errorMessage = responseData.details;
          }
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="col-span-3">
                    ...
                    <div className="space-y-3">
                      {formData.eventDates.length === 0 ? (
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            setFormData({ ...formData, eventDates: [{ date: '', time: '', title: '' }] });
                          }}
                          className="w-full"
                        >
                        ...
                        <>
                          {formData.eventDates.map((notification, index) => (
                            <div key={index} className="flex items-center gap-2 p-3 border border-slate-200 rounded-lg bg-slate-50">
                              <div className="flex items-center gap-1 px-2 py-1 bg-blue-100 rounded text-xs font-medium text-blue-700 whitespace-nowrap">
                                Notification
                              </div>
                              <Input
                                type="text"
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              setFormData({ ...formData, eventDates: [...formData.eventDates, { date: '', time: '', title: '' }] });
                            }}
                            className="w-full"
                          >
                            <Plus className="h-4 w-4 mr-2" />
                    ...
                    <p className="text-xs text-slate-500 mt-2">
                      Notifications will be sent automatically on these dates each year (use MM-DD format). Include a time and title for each notification.
                    </p>
                  </div>
                </div>
                  ...
                  {createdMcqs.map((mcq, idx) => (
                    <div key={mcq.id || idx} className="flex items-center justify-between p-2 bg-white rounded border border-slate-200">
                      <div className="flex-1">
                        <p className="text-sm font-medium text-slate-900 line-clamp-1 whitespace-pre-wrap">
                  ...
                  {mcqFormData.explanationImagePreview ? (
                    ...
                    <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                      <input
                        ...
                        className="hidden"
                        id="mcq-explanation-image-upload-gk"
                      />
                      <label
                        htmlFor="mcq-explanation-image-upload-gk"
