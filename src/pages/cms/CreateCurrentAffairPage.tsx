...
export default function CreateCurrentAffairPage() {
    ...
    onSuccess: async (response) => {
      ...
      if (formData.sendNotification && createdArticle) {
        try {
          ...
          const plainTextDescription = stripHtml(formData.description || 'Check out our latest current affair!');
          ...
          if (formData.notificationTime && formData.notificationTime.trim()) {
            ...
            await notificationJobsApi.create({
              messageType: 'ARTICLE',
              title: formData.title || 'New Current Affair',
              content: notificationBody || 'Check out our latest current affair!',
              image: imageUrl || undefined,
              redirectionLink: `https://lifeset.app/current-affairs/${articleId}`,
            ...
            await notificationsApi.send({
              ...
              notification: {
                title: formData.title || 'New Current Affair',
                body: notificationBody || 'Check out our latest current affair!',
              },
              redirectUrl: `https://lifeset.app/current-affairs/${articleId}`,
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      // Invalidate and refetch to update counts
      await queryClient.invalidateQueries({ queryKey: ['current-affairs'] });
      await queryClient.invalidateQueries({ queryKey: ['current-affair', id] });
      await queryClient.invalidateQueries({ queryKey: ['wall-categories'] });
      await queryClient.invalidateQueries({ queryKey: ['chapter-post-counts'] });
      await queryClient.refetchQueries({ queryKey: ['wall-categories', 'parents'] });
      ...
      if (formData.sendNotification && id) {
        try {
          ...
          const plainTextDescription = stripHtml(formData.description || 'Check out our updated current affair!');
          const notificationBody = plainTextDescription.length > 200 
            ? plainTextDescription.substring(0, 200) + '...' 
            : plainTextDescription;
          ...
          if (formData.notificationTime && formData.notificationTime.trim()) {
            ...
            await notificationJobsApi.create({
              messageType: 'ARTICLE',
              title: formData.title || 'Updated Current Affair',
              content: notificationBody || 'Check out our updated current affair!',
              image: imageUrl || undefined,
              redirectionLink: `https://lifeset.app/current-affairs/${id}`,
              scheduledAt: scheduledDate.toISOString(),
              language: 'ALL',
              frequency: 'ONCE',
            });
            ...
            await notificationsApi.send({
              ...
              notification: {
                title: formData.title || 'Updated Current Affair',
                body: notificationBody || 'Check out our updated current affair!',
              },
              redirectUrl: `https://lifeset.app/current-affairs/${id}`,
              imageUrl: imageUrl || undefined,
              data: {
                articleId: id,
                type: 'current-affair',
